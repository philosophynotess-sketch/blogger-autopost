...
def generate_content(category, topic):
    ...
    prompt = f"""
    당신은 15년 차 시니어 개발자이자 기술 블로거입니다.
    주제: "{topic}"에 대해 다른 개발자가 실무에 즉시 적용할 수 있는 **깊이 있는 기술 가이드와 코드 스니펫**을 작성하세요.

    --- MISSION FOR ADSENSE & SEO ---
    - '가치가 없는 콘텐츠'로 분류되지 않도록 겉핥기 식의 가벼운 설명은 배제하고, 기술적인 원리와 해결책을 심도 있게 다루세요.
    - "안녕하세요", "아직도 ~하시나요?", "결론:" 같은 뻔한 AI 말투와 서론/결론의 군더더기를 철저히 없애세요. 첫 문장부터 바로 핵심을 짚으세요.

    --- STRICT HTML STRUCTURE (FOR UI) ---
    - <div class="vibe-keypoint">: 글 최상단에 2~3줄의 명확한 기술적 요약 제공
    - <h2 id="sec1">, <h2 id="sec2"> : 기술적인 소제목
    - <pre><code class="language-python(또는 언어명)"> : 실제 동작 가능한 예제 코드와 주석을 반드시 포함할 것
    - <div class="vibe-terminal">: 에러 로그나 터미널 출력 결과를 보여줄 때 사용
    - Markdown(##, ** 등) 사용 절대 금지, 오직 HTML 태그만 사용할 것.

    [FEATURED_IMAGE_PROMPT: (High-quality thumbnail prompt in English)]
    [TAGS: 태그1, 태그2, 태그3] (주제와 관련된 핵심 기술 키워드 3개)
    
    <article>
    <h1>[자연스러운 기술 블로그 제목]</h1>
...
def post_to_blogger(title, content, image_url, category, dynamic_tags):
    ...
    <style>
      ...
      pre {{ background-color: #1e1e1e; color: #d4d4d4; padding: 20px; border-radius: 8px; overflow-x: auto; font-family: 'Consolas', 'Courier New', monospace; font-size: 0.9em; margin: 20px 0; }}
      code {{ font-family: 'Consolas', 'Courier New', monospace; background-color: #f1f5f9; color: #e53e3e; padding: 2px 6px; border-radius: 4px; font-size: 0.9em; }}
      pre code {{ background-color: transparent; color: inherit; padding: 0; }}
      .vibe-terminal {{ background-color: #000; color: #00ff00; padding: 15px; border-radius: 6px; font-family: monospace; font-size: 0.85em; margin: 20px 0; overflow-x: auto; border-left: 4px solid #00ff00; }}
      ...
      .vibe-promo {{ background-color: #f0f4f8; padding: 20px; border-radius: 10px; text-align: center; margin: 50px 0 15px 0; color: #2d3748; font-weight: 600; font-size: 0.95em; }}
      .vibe-disclaimer {{ background-color: #fff5f5; border: 1px solid #fed7d7; padding: 15px; border-radius: 8px; margin-bottom: 30px; font-size: 0.85em; color: #c53030; }}
    ...
    <div class="vibe-wrap">
      ...
      <div class="vibe-disclaimer">
        <strong>※ 주의사항:</strong> 본 포스팅에 포함된 코드 스니펫 및 AI가 제안한 해결책은 참고용이며, 실제 서비스 환경에 적용하기 전에 반드시 충분한 테스트와 검증을 거치시기 바랍니다.
      </div>
